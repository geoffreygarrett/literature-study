...
The cross term in \autoref{eq:collision_2d_gaussian} is eliminated through a coordinate rotation, which gives:
\begin{equation}
...
% and this rotation is defined by the transformation:

% \begin{equation}
%     \boldsymbol{q}_r=T \boldsymbol{q},
% \end{equation}
...
% The sign ambiguity in \autoref{eq:rotations} is resolved by ensuring that the cross term in \autoref{eq:collision_2d_gaussian:rotated} is zero, meaning that:
% \begin{equation}
%     2(f-e) \sin (\phi) \cos (\phi)+g\left[\cos (\phi)^{2}-\sin (\phi)^{2}\right]=0
% \end{equation}

% Then $h(x)$ is made to be symmetric through a scale change in the $y$ axis, which gives:
...
% The $y$ component of $\boldsymbol{q}_{r}: q_{r}(2)$ is then multiplied by this scale factor, giving:

% \begin{equation}
%     q_{r s}(2)=\sqrt{(\beta / \alpha)} q_{r}(2).
% \end{equation}

% The $y$ component of the hard-body circle is then multiplied by this scale factor, giving:
