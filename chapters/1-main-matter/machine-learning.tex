...
Given that the input-output pair is $\{\gls{x_in}_i, \mathbf{y}_i\}$,
...
\begin{itemize}
    ...
    \begin{equation}
        L_\text{MBE} = \frac{1}{N}\sum_{i=1}^{N}\sum_{j=1}^{M}(\hat{{y}}_{ij}-{y}_{ij})
    \end{equation}
    ...
    \begin{equation}
        \begin{aligned}
            L_\text{MSLE} &= \frac{1}{N}\sum_{i=1}^{N}\sum_{j=1}^{M}(\log{({{y}}_{ij}+1)} - \log{(\hat{{y}}_{ij}+1)})^2 \\
            &= \frac{1}{N}\sum_{i=1}^{N}\sum_{j=1}^{M}2\log(\frac{{{y}}_{ij}+1}{\hat{{y}}_{ij}+1})
        \end{aligned}
    \end{equation}
    ...
    bounding the error in the interval $[0,2]$. \begin{equation}
                                                    L_\text{\gls{CD}} =
                                                    \frac{1}{N}\sum_{i=1}^{N}\bigg(1-\frac{\mathbf{y}_i\cdot{}\hat{\mathbf{y}}_i}{||\mathbf{y}_i||_2\cdot{}||\hat{\mathbf{y}}_i||_2}\bigg)
    \end{equation}
\end{itemize}
...
intended to reduce its generalisation error but not its training error.
