...
Given the input-output pair $\{\gls{fn:in}, \gls{fn:out}\}$, $\glsvec{fn:out}^{\gls{ml:ith:xy}}$ is referred to as the \textit{ground truth}, and $\hat{f}(\gls{x_in_i})=\gls{y_pred_i}$ are referred to as the \textit{predictor} and \textit{predicted output} respectively. The performance in the form of a loss function \gls{J} (also referred to as an objective or cost function in literature) is then a function of $\glsvec{fn:out}^{\gls{ml:ith:xy}}$ and \gls{y_pred_i}, therefore $\gls{J}(\gls{y_pred_i}, \glsvec{fn:out}^{\gls{ml:ith:xy}})$.
...
\begin{itemize}
    ...
    \gls{SVM} classifiers \cite{Wu2007, Liu2007, Zhang2008}. The loss function assumes that \glsvec{fn:out}^{\gls{ml:ith:xy}}_\gls{ix:sub} and
    ...
    ($\glsvec{fn:out}^{\gls{ml:ith:xy}}_\gls{ix:sub}\cdot{}\gls{y_pred_ij}$). This provides better gradient
...
complex tasks, and those with excess capacity will perform complex tasks, but
...
intuitively related to the model's propensity to underfit and overfit a set of
training data.
...
\begin{figure}[htp]
    ...
    \caption{
        Three different models fit an arbitrary example training set
...
A fundamental approach to regularization in \gls{ML} originating from \textit{statistical learning} is the use of \textbf{parameter norm penalties}. This technique aims at guiding the parameter vectors through the addition of a penalty term in the objective (cost/loss) function being minimised. This penalty constrains (regularises) or shrinks the optimal \gls{w_vec} estimate towards zero, effectively discouraging a learning algorithm from producing an excessively complex model to avoid overfitting. Norm penalty methods can be generalised as:
