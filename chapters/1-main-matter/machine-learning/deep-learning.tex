...
\gls{b})=\gls{x_in}^T\gls{w_vec}+\gls{b}$; $\gls{x_in}\in\mathbb{R}^{\gls{n_x}}$,
$\gls{w_vec}\in\mathbb{R}^{\gls{n_x}}$, $\gls{b}\in\mathbb{R}$.
...
\subsection{\Glsfmtlong{MLPs} (\glsfmtshort{MLPs})}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{equation}
    \mathbf{h}^{[1]} = g^{[1]} \bigg(\mathbf{W}^{{[1]}\;T}\gls{x_in} + \mathbf{b}^{[1]}\bigg);
\end{equation}

\begin{equation}
    \mathbf{h}^{[2]} = g^{[2]} \bigg(\mathbf{W}^{{[2]}\;T}\mathbf{h}^{[2]} + \mathbf{b}^{[2]}\bigg);
\end{equation}
